...
<div
  class="flex items-center justify-between py-[3px] gap-1.5 border-b border-hud-border-subtle last:border-b-0"
>
  <!-- Left: label + help -->
  ...
  {#if locked}
    ...
    <div class="flex items-center gap-1">
      <span class="text-[11px] font-semibold text-hud-locked tracking-wide">••••</span>
      <button
  ...
  {:else}
    ...
    <!-- Right: value -->
    <span
      class={`shrink-0 text-[11px] font-semibold tabular-nums whitespace-nowrap ${
        status === "ok"
          ? "text-hud-ok"
          : status === "warn"
            ? "text-hud-warn"
